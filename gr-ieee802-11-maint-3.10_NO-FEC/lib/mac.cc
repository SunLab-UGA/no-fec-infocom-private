...
public:
    ...
    void phy_in(pmt::pmt_t msg)
    {
        ...
        // use new debug function
        DEBUG_MSG("pdu len " << pmt::blob_length(blob));
    ...
    void app_in(pmt::pmt_t msg)
    {
        ...
        if (msg_len > MAX_PAYLOAD_SIZE) {
            // DEBUG_MSG("Jumbo frame detected");
    ...
    void generate_mac_data_frame(const char* msdu, int msdu_size, int* psdu_size)
    {
        ...
        header.duration = 0x0000; // in microseconds (not used here...)

        for (int i = 0; i < 6; i++) {
            header.addr1[i] = d_dst_mac[i]; // destination address
            header.addr2[i] = d_src_mac[i]; // source address
            header.addr3[i] = d_bss_mac[i]; // BSSID
        }
...
// {
//     if (src_mac.size() > std::vector<uint8_t>::max_size() ||
//         dst_mac.size() > std::vector<uint8_t>::max_size() ||
//         bss_mac.size() > std::vector<uint8_t>::max_size()) {
...
{
    if (src_mac.size() > src_mac.max_size()) {
        std::cout << "src_mac vector size exceeds maximum allowed size" << std::endl;
        throw std::invalid_argument("src_mac vector size exceeds maximum allowed size");
    }
    if (dst_mac.size() > dst_mac.max_size()) {
        std::cout << "dst_mac vector size exceeds maximum allowed size" << std::endl;
        throw std::invalid_argument("dst_mac vector size exceeds maximum allowed size");
    }
    if (bss_mac.size() > bss_mac.max_size()) {
        std::cout << "bss_mac vector size exceeds maximum allowed size" << std::endl;
        throw std::invalid_argument("bss_mac vector size exceeds maximum allowed size");
    }
